...
import { ChevronLeft, Plus, Trash2, Clock, Link2, Flame, Zap, ClipboardList, Timer, NotebookPen } from 'lucide-react-native';
                                    ...
                                    <View className="flex-row items-center gap-2">
                                        {currentEx.sets.length > 0 && (
                                            <TouchableOpacity
                                                ...
                                                className={`p-1.5 rounded-lg ${allHaveRest ? 'bg-emerald-500/10' : ''}`}
                                            >
                                                <ClipboardList size={18} color={allHaveRest ? "#10B981" : "#9CA3AF"} />
                                            </TouchableOpacity>
                                        )}
                                        <TouchableOpacity
                        ...
                        {state.activeExerciseIndex !== null && (
                            <View className="pb-8">
                                ...
                                <View className="flex-row mt-4 items-center pl-10 pr-2">
                                    <Text className="flex-1 text-[10px] font-bold text-gray-400 tracking-wider">OPCIONES</Text>
                                    <View className="w-16 items-center"><Text className="text-[10px] font-bold text-gray-400 tracking-wider">KG</Text></View>
                                    <View className="w-16 items-center"><Text className="text-[10px] font-bold text-gray-400 tracking-wider">REPS</Text></View>
                                </View>
                                            ...
                                            <View className="flex-row items-center py-1 pr-2">
                                                <View className="justify-center w-10 items-center">
                                                    <TouchableOpacity
                                                        ...
                                                        className={`w-7 h-7 rounded-lg items-center justify-center transition ${set.type === 'warmup' ? 'bg-orange-500/20' :
                                                            set.type === 'failure' ? 'bg-red-500/20' :
                                                                'bg-[#1C1C22] border border-gray-800'
                                                            }`}
                                                    >
                                                        {set.type === 'normal' && <Text className="font-bold text-[11px] text-gray-400">{setIdx + 1}</Text>}
                                                        {set.type === 'warmup' && <Flame size={14} color="#FB923C" />}
                                                        {set.type === 'failure' && <Zap size={14} color="#F87171" />}
                                                    </TouchableOpacity>
                                                </View>
                                                <View className="flex-1 flex-row gap-4 items-center">
                                                    <TouchableOpacity onPress={() => actions.setActiveSetForRest(setIdx)}>
                                                        <Timer size={16} color={set.restTime.h !== '00' || set.restTime.m !== '00' || set.restTime.s !== '00' ? "#10B981" : "#6B7280"} />
                                                    </TouchableOpacity>
                                                    <TouchableOpacity onPress={() => actions.setActiveSetForNote(setIdx)}>
                                                        <NotebookPen size={16} color={set.note && set.note.trim() !== '' ? "#10B981" : "#6B7280"} />
                                                    </TouchableOpacity>
                                                </View>
                                ...
                                <TouchableOpacity
                                    ...
                                    className="w-full bg-[#1C1C22] border border-gray-800 py-3.5 rounded-xl flex-row items-center justify-center gap-2 mt-4"
                                >
                                    <Plus size={16} color="#D1D5DB" />
                                    <Text className="text-gray-300 text-sm font-medium">Añadir serie</Text>
                                </TouchableOpacity>
                            </View>
                        )}
                    </View>
